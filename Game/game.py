...
def remove_punct(text): #Complete
...
def remove_spaces(text): #Complete
    ...
    while trailing_space == True:
        if text[len(text) - counter - 1] == " ":
            counter = counter + 1

        else:
            trailing_space = False
            new_text = new_text[0:len(new_text) - counter]

    #print("'" + new_text + "'")
    return(new_text)


def normalise_input(user_input): #Complete
    ...
    if new_text.count(" ") == len(new_text):
        leading_space = False
        trailing_space = False
        new_text = ""
    ...
    while leading_space == True:
        if new_text[counter] == " ":
            ...
            leading_space = False
            new_text = new_text[counter:]
            counter = 0

    while trailing_space == True:
        if new_text[len(new_text) - counter - 1] == " ":
            counter = counter + 1
    ...
    user_input = new_text.lower()

    #print("'" + new_text + "'")
    #return(new_text)
    return(user_input)
    
def display_room(room): #Complete
...
def exit_leads_to(exits, direction): #Complete
...
def print_menu_line(direction, leads_to): #Complete
...
def is_valid_exit(exits, user_input): #Complete
...
def menu(exits):
    ...
    while True:
        ...
        is_valid_exit(exits, user_input)
        if True:
            return(user_input)
        else:
            print("Error")
...
def move(exits, direction): #complete
